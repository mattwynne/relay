...
class ConcurrentRunner extends Runner {
    ...
    {
        parent::__construct($host, $port, $auth, $filter, $verbose);

        if ($workers < 2) {
            throw new \Exception("Invalid number of workers ($workers <= 1)\n");
        }
        if ($duration < 1.0) {
            throw new \Exception("Invalid duration ($duration <= 1.0)\n");
        }
    ...
    protected function runConcurrent($reporter, $subject, $class, $method) {
        ...
        $cmd1 = array_sum($this->getRedisCommandCount());
        ...
        $cmd2 = array_sum($this->getRedisCommandCount());
        ...
        $reporter->finishedTimedSubject($subject, $tot_ops, ($end - $start) / 1e+6, $cmd2 - $cmd1);
    }
